...
public class UserService {
    ...
    public void checkUserLogin(String email, String password) {
        var maybeUser = repository.findByEmail(email);
        if (!maybeUser.map(user -> passwordEncoder.matches(password, user.getPasswordHash()))
                .orElse(false)) {
            throw new UserNotFoundException("User with email " + email + " not found or password is incorrect");
        }
    }

    public void registerUser(UserRegDto userRegDto) {
        if (repository.existsByLogin(userRegDto.login())) {
            throw new UserAlreadyExistsException("User with email " + userRegDto.login() + " already exists");
        }
        ...
        var encryptedPassword = passwordEncoder.encode(userRegDto.password());
